...
static char current_host_os[30] = "unknown";
...
static void process_raw_hid_data(uint8_t *data) {
    ...
    case _OS:
        ...
        if (os_name_length > sizeof(notification.host_os) - 1) {
            os_name_length = sizeof(notification.host_os) - 1;
        }

        memcpy(notification.host_os, &data[2], os_name_length);
        notification.host_os[os_name_length] = '\0';

        strncpy(current_host_os, notification.host_os, sizeof(current_host_os) - 1);
        current_host_os[sizeof(current_host_os) - 1] = '\0';
